...
export function createNavigationMapboxPresenter(
  ...
  return Object.freeze({
    update(snapshot, force = false): boolean {
      ...
      const bearingChanged =
        lastBearing === null ? Infinity : bearingDifference(snapshot.bearing, lastBearing);
      const zoomChanged =
      ...
      options.map.easeTo({
        center: [snapshot.visualLocation.longitude, snapshot.visualLocation.latitude],
