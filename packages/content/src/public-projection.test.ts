...
describe("offline content snapshot", () => {
  ...
  it("rejects content published after snapshot generation", () => {
    const futurePublished = published({
      id: "place-future",
      locale: "pt-BR",
    ...
    expect(
      createOfflineContentSnapshot([futurePublished], {
        destinationId: "morro-de-sao-paulo",
        generatedAt: "2026-09-20T10:04:59.000Z",
        expiresAt: "2026-09-20T12:00:00.000Z",
      }),
    ).toBeNull();

    const valid = createOfflineContentSnapshot([futurePublished], {
      destinationId: "morro-de-sao-paulo",
      generatedAt: "2026-09-20T10:10:00.000Z",
      expiresAt: "2026-09-20T12:00:00.000Z",
    });
    if (!valid) throw new Error("Expected valid snapshot fixture.");
  ...
  it("requires a bounded freshness window", () => {
    ...
    expect(
      isOfflineContentSnapshotFresh(snapshot, "2026-09-20T11:59:59.000Z"),
    ).toBe(true);
    expect(
      isOfflineContentSnapshotFresh(snapshot, "2026-09-20T12:00:00.000Z"),
    ).toBe(false);
  });
