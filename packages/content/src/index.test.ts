...
describe("content versioning", () => {
  it("rejects revisions older than the current lifecycle clock", () => {
    expect(
      reviseContent(
        draft,
        { summary: "Descrição retroativa proibida." },
    ...
    expect(
      reviseContent(
        revised!,
        { summary: "Relógio não pode voltar." },
        "2026-09-20T10:04:59.000Z",
      ),
    ).toBeNull();
  });
...
describe("content lifecycle", () => {
  it("rejects backdated publish, archive, and schedule transitions", () => {
    const beforeCreation = "2026-09-20T09:59:59.000Z";

    expect(
      transitionContent(draft, {
        status: "published",
        transitionedAt: beforeCreation,
      }),
    ).toBeNull();
    ...
    expect(
      transitionContent(draft, {
        status: "scheduled",
        transitionedAt: beforeCreation,
        scheduledFor: "2026-09-20T10:30:00.000Z",
      }),
    ).toBeNull();
    ...
    expect(
      transitionContent(preview!, {
        status: "published",
        transitionedAt: "2026-09-20T10:04:59.000Z",
      }),
    ).toBeNull();
  });
