import type { ContentDocument, ContentFields, ContentKind } from "./index.js";
...
export function createOfflineContentSnapshot(
  ...
  for (const source of documents) {
    ...
    if (Date.parse(document.publishedAt) > Date.parse(input.generatedAt)) {
      return null;
    }
...
export function parseOfflineContentSnapshot(
  ...
  for (const source of documents) {
    ...
    if (Date.parse(document.publishedAt) > Date.parse(generatedAt)) return null;
