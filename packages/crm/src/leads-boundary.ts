...
function normalizeCreate(
  ...
  const optionalEmail =
    input.email === undefined || input.email === ""
      ? undefined
      : safeEmail(input.email);
  if (input.email !== undefined && input.email !== "" && !optionalEmail) {
    return null;
  }
  const monthlyValue =
    input.monthlyValue === undefined || input.monthlyValue === ""
      ? undefined
      : safeMoney(input.monthlyValue);
  if (
  ...
  if (optionalEmail !== undefined) record.email = optionalEmail;
  if (monthlyValue !== undefined) record.monthlyValue = monthlyValue;
  return record;
}

function normalizeUpdate(
  input: CrmLeadUpdateInput,
): CrmLeadUpdateRecord | null {
  const patch: {
  ...
  if (input.monthlyValue !== undefined) {
    const monthlyValue =
      input.monthlyValue === "" ? "" : safeMoney(input.monthlyValue);
...
export class CrmLeadServerBoundary {
  ...
  async updateStage(
    ...
    if (!existing)
      return this.reject("lead.update_stage", session, "not_found", id);
