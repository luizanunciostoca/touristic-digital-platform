...
The existing `.github/workflows/ticketing-m147-contract.yml` path-scopes `packages/ticketing/**` and `services/ticketing/**`. After CI V2 PR #255, that workflow provisions MySQL 8.4 in parallel with static validation, builds the Ticketing runtime graph once and runs the complete Ticketing server Vitest suite. M150's reservation MySQL integration file is part of that complete suite, so the optimized runner preserves the new concurrency and persistence coverage without a duplicate focused invocation. The package-level `test:integration` script also continues to include both the original M147 persistence suite and the M150 reservation suite for direct local/targeted execution.
