...
This evidence records what is implemented and statically auditable on `feat/payments-mercado-pago-render-v2` before any claim of provider verification. The exact head is tracked in PR #293 rather than embedded here, so this evidence cannot stale itself when updated.
