# CRM Administrativo — Migration Matrix (M138 reconciliation)
...
- M138 branch base: `main@5e659459b2a3c81939cdca056a5b98ed2f4f120d` after merged M137;
- current feature status: `migrating`, not `equivalent` and not `released`.

M73–M133 established real platform Auth/Node composition, MySQL persistence across the commercial aggregates, schedulers, public token flows and authenticated browser mutation lifecycles. M134–M136 hardened public contract signature pointer handling, M137 removed the duplicate Meetings shell lifecycle, and M138 restores the frozen dashboard metric/funnel contract as an authenticated server-authoritative surface. This matrix remains conservative where the frozen V1 contract is not fully reproduced or where a formal visual/release gate is still open.

| Contract | Frozen V1 evidence | V2 state at M138 | Status | Remaining decision |
| --- | --- | --- | --- | --- |
| CRM application shell and authenticated layout | React client + `CRMLayout`/dashboard layout | `apps/admin-crm/public/index.html` provides authenticated shell, navigation and dedicated CRM surfaces; M104/M105 bind platform session; M137 keeps Meetings on its canonical dedicated surface | PASS | Keep CRM browser auth owned by platform Auth. |
...
| Dashboard metrics and funnel | `Dashboard.tsx`; protected `metrics.funnel`; `getFunnelMetrics()` | M138 freezes V1 metric formulas, derives the snapshot from persisted Leads/interactions, exposes authenticated GET-only `/api/crm/metrics/funnel`, renders KPIs/funnel/recent activity and refreshes every 30 seconds | PASS | Keep all calculations server-authoritative and read-only. |
...
| Meetings lifecycle | `Meetings.tsx`; create/complete/no-show/cancel | M74/M75 domain, persistence and HTTP composition; M110/M120+ browser consultation, scheduling and result lifecycle; M137 removes the stale duplicate shell implementation | PASS | Preserve explicit auditable status transitions. |
...
| Tokenized contract public view | `/contracts/view/:token` | M82 public sign boundary; M118/M119 browser route; M132–M136 pointer/capture/clamping hardening | PASS | Keep signature payload bounded and public projection minimal. |
...
| Persistence model | Drizzle ORM + MySQL via `server/db.ts` | `@touristic/crm-server` persists leads, meetings, proposals, contracts, follow-ups, trials, referrals, interactions and durable audit in MySQL; M138 reads metrics from the same authority without new tables | PASS | Historical data migration/reconciliation remains a deployment concern. |
...
| Server-side audit/authorization | protected tRPC procedures + host auth | platform Auth, CRM authorization and durable MySQL audit are composed across authenticated transports; M138 audits denied metric reads through the same ledger | PASS | Continue emitting structured denials and mutation evidence. |
| Automated regression coverage | `server/crm.test.ts`, auth tests | permanent domain/persistence/transport tests plus browser contract proofs cover the migrated aggregates and M138 dashboard authority | PASS | Aggregate critical CRM browser gates into release GO criteria. |
| Responsive/accessibility visual surface | Radix/Tailwind CRM pages | browser surfaces use semantic controls/status states and focused contracts, but no complete frozen V1 visual/accessibility equivalence matrix is consolidated | PARTIAL | Capture consolidated responsive/accessibility/visual evidence before `equivalent`. |

## M138 reconciled score
...
The matrix moved from M72 (`0 PASS / 8 PARTIAL / 17 GAP`) to M133 (`17 PASS / 5 PARTIAL / 3 GAP`) and now to M138 (`18 PASS / 5 PARTIAL / 2 GAP`). `FEATURE-0006` remains `migrating` because dashboard metrics were only one of the remaining closure items.

## Remaining CRM closure order

1. Complete Lead detail/activity and remaining CRUD parity.
2. Close or explicitly reclassify Follow-up send/respond behavior.
3. Inventory and implement generic CRM settings.
4. Decide the frozen V1 object-storage requirement and add a server-only adapter if still necessary.
5. Freeze the CRM-owned AI-assisted contract instead of coupling directly to a provider.
6. Consolidate responsive/accessibility/visual equivalence evidence.
7. Run release-candidate, staging, rollback and production-readiness gates separately from migration equivalence.
