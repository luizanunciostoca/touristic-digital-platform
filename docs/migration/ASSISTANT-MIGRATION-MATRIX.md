...
| weather/location/photos/price/hours   | `intent-engine.js`         | `src/intent-engine.ts` + `src/domain-handlers.ts`                                                               | PARTIAL    | weather, geolocalização, photos e hours conectados; assets binários de photos e paridade completa de weather/i18n ainda pendentes        |
...
| Handlers de domínio do diálogo        | `assistant-dialog.js`      | `src/domain-handlers.ts` + portas do `src/dialog-controller.ts`                                                 | PARTIAL    | weather/location/nearby/favorites/help/photos/hours/more_info conectados; assets binários de photos e paridades de i18n restantes faltam |
...
| Voz                                   | `voice/**`                 | `src/voice-synthesis.ts` + `assistant-voice-adapter.ts` + `assistant-voice-input-adapter.ts`                    | PARTIAL    | síntese, preferências e entrada por voz PT/EN/ES/HE conectadas; seletor/configuração visual ainda pendente                               |
