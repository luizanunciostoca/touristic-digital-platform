...
| Contrato V1                         | Evidência V1               | Destino V2              | Estado M3 | Critério de PASS                                                      |
| ----------------------------------- | -------------------------- | ----------------------- | --------- | --------------------------------------------------------------------- |
...
| NLP local-first                     | `intent-engine.js`         | `src/intent-engine.ts`  | PARTIAL   | padrões diretos LLM-first restantes + handlers de domínio             |
...
| Fallback LLM — execução             | `llm-fallback.js`          | a implementar           | GAP       | chamada real somente quando necessária, com resposta equivalente      |
...
## Estado do milestone M3

O M3 porta a política completa de decisão do fallback LLM da V1 para um boundary explícito e testável em `src/llm-policy.ts`. Foram preservados: intents que sempre exigem LLM, threshold de confiança `0.5`, flag explícita `requiresLLM`, contagem das cinco dimensões semânticas, padrões de complexidade e threshold estritamente maior que `90` caracteres.

Este milestone **não** implementa ainda o provedor LLM nem o endpoint `/api/ai/*`; portanto, separar decisão de execução mantém a arquitetura segura e evita qualquer segredo de provedor no browser.
