...
A valid event whose Payment is unknown is durably accepted with `matched=false` and HTTP 202, preserving non-leaking V1 semantics and evidence for later reconciliation. M141 never mutates Payment, emits a Financial domain outcome or activates Business. That authoritative state application is M142.

## Matrix

| Contract                                | Frozen V1 / architecture evidence                                                               | V2 state at M141                                                                                                                                                                      | Status  | Migration decision                                                                                           |
| --------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------ |
...
| Webhook replay/idempotency              | repeated `CONFIRMED` record does not reconvert                                                  | MySQL claims provider event ID with signed payload hash; exact replay returns the first receipt and divergent reuse fails without overwrite                                           | PASS    | M142 may apply only a newly claimed matched event and must preserve out-of-order evidence.                   |
| Payment state authority                 | paid/approved/confirmed -> `CONFIRMED`; non-paid does not promote                               | transition guards and repository CAS exist; M141 persists verified provider evidence but deliberately performs no Payment mutation                                                    | PARTIAL | M142 composes verified event, transition rules and authoritative result atomically/idempotently.             |
...
| Financial audit/observability           | architecture requires audit/metrics; M134 covers provider-cost ops only, not product money      | M141 adds structured verified/denied/replay/collision webhook audit without raw body, secret, provider reference or PII; durable central audit/metrics remain absent                  | PARTIAL | Add durable/central observability around state application and ledger operations.                            |
...
| Rollback/migration strategy             | release process requires migration and rollback                                                 | M141 adds one append-only event table and a versioned callback; disabling the route leaves Orders, Payments and immutable provider evidence intact                                    | PARTIAL | Keep the table for forensic/reconciliation rollback; use expand/contract before outcome application.         |
...
M141 closes raw-body webhook authenticity, non-leaking unmatched acknowledgement and durable provider-event replay. Payment state application, Business result composition, operational ledger, reconciliation, settlement, subscriptions and browser execution remain below PASS.

## Promotion decision

After M141 and green Quality, Payments Persistence Integration, Sandbox Provider and Verified Webhook gates on the final head:

- `FEATURE-0009` and `MIG-0010` remain `migrating`;
- behavior/visual/API equivalence flags remain `false`;
- only a cryptographically verified event can be durably claimed;
- no provider outcome changes Payment or Business in this milestone;
- no real money movement is enabled.

## Next milestone

M142 must compose the verified provider event into an idempotent Payment outcome service:

1. only newly claimed, matched and verified events may be applied;
2. provider status maps through the explicit Payment transition table;
3. stale/out-of-order terminal conflicts preserve evidence without rollback;
4. `PaymentApproved`/failure results are emitted through a durable boundary;
5. Business consumes only the authoritative verified result;
6. status projections remain non-PII and never infer success from browser return.

Operational ledger posting/refund/reconciliation remain later milestones. Affiliates stays blocked.
