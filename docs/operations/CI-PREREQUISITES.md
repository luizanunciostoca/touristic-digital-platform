...
`pnpm ci:quality` executes the full repository `check`: formatting, architecture/Platform contracts, Feature Registry, CI governance, strict workflow supply-chain enforcement, lint, typecheck, test and build.

For diagnostics only, `pnpm ci:supply-chain` reports the supply-chain inventory. `pnpm ci:supply-chain:strict` is the enforced contract and fails if any external GitHub Action reference is not an immutable 40-character commit SHA, or if unsafe constructs such as `pull_request_target`, `permissions: write-all`, missing explicit top-level permissions, or `secrets: inherit` are found.
...
Permanent workflows default to:
...
All 39 workflow files in the stacked tree were included in the hardening pass. External Actions are pinned to reviewed immutable commits, including:
...
The strict checker intentionally covers external `uses:` references. Container service images such as `mysql:8.4` remain tag-pinned, not digest-pinned; digest migration requires separate compatibility validation and is not silently conflated with Action pinning.

## Repository secret names

The current workflow audit found the following repository-level CI secret name:
...
Browser evidence workflows may install Playwright/Chromium and remain path-scoped. Persistence/integration workflows may require their declared MySQL service. These specialized contracts are permanent evidence but are not globally required branch-protection contexts.
...
4. Run exact-head `quality`; strict supply-chain enforcement is part of that gate.
5. Run every affected path-scoped Auth/Business/CRM/Payments/Ticketing/browser evidence contract that should trigger for the diff.
