...
const output = path.resolve(process.argv[3] ?? 'docs/migration/generated/v1-design-system-inventory.json');
const cssFiles = [];
...
async function walk(directory) {
  for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
    ...
    else if (/\.(css|scss|sass|less)$/i.test(entry.name)) cssFiles.push(fullPath);
  }
}
...
const inventory = {
  ...
  source,
  files: [],
  totals: { files: 0, colors: 0, fontFamilies: 0, mediaQueries: 0, animations: 0, customProperties: 0 },
};
...
for (const file of cssFiles.sort()) {
  ...
  const record = {
    ...
    colors: [...new Set(count(/#[0-9a-fA-F]{3,8}\b|(?:rgb|hsl)a?\([^)]*\)/g, text))].sort(),
    fontFamilies: [...new Set(count(/font-family\s*:\s*[^;}]+/gi, text).map((value) => value.split(':').slice(1).join(':').trim()))].sort(),
    mediaQueries: count(/@media\b/g, text).length,
    animations: count(/@keyframes\s+[\w-]+/g, text),
    customProperties: [...new Set(count(/--[\w-]+\s*:/g, text).map((value) => value.slice(0, -1)))].sort(),
...
console.log(`Inventário criado: ${output} (${inventory.totals.files} arquivos CSS)`);
