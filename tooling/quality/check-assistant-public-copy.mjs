...
const forbidden = [
  {
    pattern: /Categoria selecionada:/gu,
    label: "PT technical category status",
  },
  { pattern: /Selected category:/gu, label: "EN technical category status" },
  {
    pattern: /Categoría seleccionada:/gu,
    label: "ES technical category status",
  },
  { pattern: /הקטגוריה שנבחרה:/gu, label: "HE technical category status" },
  {
    pattern: /Ações disponíveis estão prontas/giu,
    label: "PT technical action status",
  },
  {
    pattern: /available actions .* are ready/giu,
    label: "EN technical action status",
  },
  {
    pattern: /acciones disponibles .* están listas/giu,
    label: "ES technical action status",
  },
  {
    pattern: /הפעולות הזמינות .* מוכנות/gu,
    label: "HE technical action status",
  },
  { pattern: /Navegação ativa/giu, label: "PT technical navigation status" },
  {
    pattern: /Navigation .* is active/giu,
    label: "EN technical navigation status",
  },
  {
    pattern: /La navegación .* está activa/giu,
    label: "ES technical navigation status",
  },
...
for (const file of publicCopyFiles) {
  ...
  for (const rule of forbidden) {
    for (const match of source.matchAll(rule.pattern)) {
      ...
      violations.push(
        `${file}:${line}: ${rule.label}: ${JSON.stringify(match[0])}`,
      );
    }
  }
}
