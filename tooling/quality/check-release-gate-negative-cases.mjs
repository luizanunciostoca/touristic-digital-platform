...
for (const marker of [
  ...
  assert.ok(pages.includes(marker), `Pages fail-closed marker missing: ${marker}`);
}
assert.ok(
...
const acceptance = await source(".github/workflows/final-release-acceptance.yml");
for (const marker of [
...
for (const path of [
  ...
  assert.ok(
    workflow.includes(".source_sha == $sha and .tree_sha == $tree and .digest == $digest"),
