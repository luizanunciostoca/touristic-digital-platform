...
const meetingsView=document.querySelector("#meetings-view");
...
const meetingsStatus=document.querySelector("#meetings-status");
const meetingsTable=document.querySelector("#meetings-table");
const meetingsBody=document.querySelector("#meetings-body");
const meetingsCount=document.querySelector("#meetings-count");
...
const meetingStatusLabels={scheduled:"Agendada",done:"Concluída",cancelled:"Cancelada",no_show:"Não compareceu"};
const meetingModalityLabels={in_person:"Presencial",online:"Online"};
...
function meetingDate(value){const date=new Date(value);return Number.isFinite(date.getTime())?new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(date):"—";}
function setStatus(message){if(leadsStatus)leadsStatus.textContent=message;}
function setMeetingsStatus(message){if(meetingsStatus)meetingsStatus.textContent=message;}
...
function stageControl(lead){const wrapper=document.createElement("div");wrapper.className="lead-stage-action";const select=document.createElement("select");select.className="lead-stage-select";select.dataset.leadId=String(lead.id);select.setAttribute("aria-label",`Etapa de ${lead.companyName}`);for(const [value,label] of Object.entries(stageLabels)){const option=document.createElement("option");option.value=value;option.textContent=label;option.selected=value===lead.stage;select.append(option);}const save=document.createElement("button");save.type="button";save.className="lead-stage-button";save.dataset.leadId=String(lead.id);save.textContent="Salvar etapa";wrapper.append(select,save);return wrapper;}
function actionCell(lead){const cell=document.createElement("td");cell.className="lead-action-cell";const button=document.createElement("button");button.type="button";button.className="lead-edit-button";button.dataset.leadId=String(lead.id);button.textContent="Editar";cell.append(button,stageControl(lead));return cell;}
...
function renderMeetings(meetings){if(!(meetingsBody instanceof HTMLElement)||!(meetingsTable instanceof HTMLElement))return;meetingsBody.replaceChildren();for(const meeting of meetings){const row=document.createElement("tr");row.append(textCell(meeting.title),textCell(meeting.leadId),textCell(meetingDate(meeting.scheduledAt)),textCell(meetingModalityLabels[meeting.modality]||meeting.modality),textCell(meetingStatusLabels[meeting.status]||meeting.status));meetingsBody.append(row);}meetingsTable.hidden=meetings.length===0;if(meetingsCount)meetingsCount.textContent=`${meetings.length} ${meetings.length===1?"reunião":"reuniões"}`;setMeetingsStatus(meetings.length===0?"Nenhuma reunião encontrada.":`Exibindo ${meetings.length} ${meetings.length===1?"reunião":"reuniões"}.`);}
...
async function loadMeetings(){if(meetingsLoaded)return;setMeetingsStatus("Carregando reuniões…");try{const response=await auth.secureFetch("/api/crm/meetings",{headers:{Accept:"application/json"}});if(!response.ok){if(response.status!==401)setMeetingsStatus("Não foi possível carregar as reuniões.");return;}const payload=await response.json();if(!payload||!Array.isArray(payload.data)){setMeetingsStatus("Resposta de reuniões inválida.");return;}renderMeetings(payload.data);meetingsLoaded=true;}catch{setMeetingsStatus("Não foi possível carregar as reuniões.");}}
...
async function updateLeadStage(id,select,button){const lead=leadsById.get(String(id));if(!lead)return;const stage=select.value;if(stage===lead.stage){setStatus("A etapa selecionada já é a etapa atual.");return;}button.disabled=true;setStatus("Atualizando etapa…");try{const response=await auth.secureFetch(`/api/crm/leads/${id}/stage`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({stage})});if(!response.ok){if(response.status!==401)setStatus(response.status===403?"Você não possui permissão para alterar etapas.":"Não foi possível atualizar a etapa.");return;}const result=await response.json();if(!result||!result.data){setStatus("Resposta de etapa inválida.");return;}leadsLoaded=false;await loadLeads();setStatus("Etapa atualizada com sucesso.");}catch{setStatus("Não foi possível atualizar a etapa.");}finally{button.disabled=false;}}
function route(){const hash=window.location.hash;const view=hash==="#leads"?"leads":hash==="#meetings"?"meetings":"dashboard";if(dashboardView instanceof HTMLElement)dashboardView.hidden=view!=="dashboard";if(leadsView instanceof HTMLElement)leadsView.hidden=view!=="leads";if(meetingsView instanceof HTMLElement)meetingsView.hidden=view!=="meetings";for(const link of viewLinks)link.classList.toggle("active",link.getAttribute("data-view-link")===view);if(view==="leads")void loadLeads();else closeLeadEditor();if(view==="meetings")void loadMeetings();}
...
leadsBody?.addEventListener("click",(event)=>{const target=event.target;if(!(target instanceof HTMLElement))return;const stageButton=target.closest(".lead-stage-button");if(stageButton instanceof HTMLButtonElement){const id=stageButton.dataset.leadId;const action=stageButton.closest(".lead-stage-action");const select=action?.querySelector(".lead-stage-select");if(id&&select instanceof HTMLSelectElement)void updateLeadStage(id,select,stageButton);return;}const button=target.closest(".lead-edit-button");if(!(button instanceof HTMLButtonElement))return;const id=button.dataset.leadId;if(id)openLeadEditor(id);});
