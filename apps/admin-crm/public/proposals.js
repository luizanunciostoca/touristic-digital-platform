...
const statusLabels = { draft: "Rascunho", sent: "Enviada", viewed: "Visualizada", accepted: "Aceita", rejected: "Recusada" };
...
function money(value) {
  ...
  return Number.isFinite(parsed) ? new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(parsed) : value;
}

function dateLabel(value) {
  if (!value) return "—";
  const date = new Date(value);
  return Number.isFinite(date.getTime()) ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short" }).format(date) : "—";
}
...
function actionCell(proposal) {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      const response = await auth.secureFetch(`/api/crm/proposals/${proposal.id}/send`, { method: "POST", headers: { Accept: "application/json" } });
      if (!response.ok) {
...
function renderProposals(proposals) {
  ...
  for (const proposal of proposals) {
    ...
    row.append(textCell(proposal.title), textCell(proposal.leadId), textCell(proposal.planName), textCell(money(proposal.monthlyValue)), textCell(money(proposal.setupFee)), textCell(`${proposal.trialDays ?? 0} dias`), textCell(statusLabels[proposal.status] || proposal.status), textCell(dateLabel(proposal.validUntil)), actionCell(proposal));
    body.append(row);
  }
  table.hidden = proposals.length === 0;
  if (count) count.textContent = `${proposals.length} ${proposals.length === 1 ? "proposta" : "propostas"}`;
  setStatus(proposals.length === 0 ? "Nenhuma proposta encontrada." : `Exibindo ${proposals.length} ${proposals.length === 1 ? "proposta" : "propostas"}.`);
}

async function loadProposals() {
  setStatus("Carregando propostas…");
  try {
    const response = await auth.secureFetch("/api/crm/proposals", { headers: { Accept: "application/json" } });
    if (!response.ok) {
...
if (createForm instanceof HTMLFormElement) {
  createForm.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await auth.secureFetch("/api/crm/proposals", { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(payload) });
      if (!response.ok) {
        if (createStatus) createStatus.textContent = "Não foi possível criar a proposta.";
        return;
      }
      createForm.reset();
