...
export function createCatalogRuntime(pool) {
  ...
  async function publishedSnapshot(place) {
    const [rows] = await pool.execute(
  ...
  async function backfillPublishedSnapshots() {
    ...
    for (const row of rows) {
      const [existing] = await pool.execute(
        `SELECT 1 FROM catalog_public_snapshots
          WHERE place_id = ? AND place_revision = ? LIMIT 1`,
  ...
  async function listActionContext(place) {
    const snapshot = await publishedSnapshot(place);
    if (!snapshot) {
      return Object.freeze({
