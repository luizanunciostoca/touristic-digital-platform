...
describe("payments startup failure diagnostics", () => {
  it("classifies bounded nested causes without exposing arbitrary messages", () => {
    ...
    expect(
      safeStartupFailureCode(
        new Error("MERCADO_PAGO_CHECKOUT_ORIGINS is invalid"),
      ),
    ...
    expect(
      safeStartupFailureCode(
        new Error("MERCADO_PAGO_WEBHOOK_SECRET is required"),
      ),
    ).toBe("MERCADO_PAGO_WEBHOOK_SECRET_REQUIRED");
    expect(
      safeStartupFailureCode(
  ...
  it("stays fail-closed when operational configuration is absent", async () => {
    ...
    expect(audit).toContainEqual(
      expect.objectContaining({
        action: "checkout.runtime",
        result: "failure",
        reason: "ORDERING_DATABASE_URL_REQUIRED",
      }),
    );
