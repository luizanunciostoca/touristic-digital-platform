...
export const SURFACES = Object.freeze([
  ...
  {
    id: "business-360",
    route: "#businesses:toca-do-morcego",
    view: "businesses",
    entity360: true,
  },
  ...
  {
    id: "affiliate-360",
    route: "#affiliates:affiliate-visual-001",
    view: "affiliates",
    entity360: true,
  },
  ...
  {
    id: "user-360",
    route: "#users:business-owner-visual",
    view: "users",
    entity360: true,
  },
...
export const MANUAL_HASHES = Object.freeze({
  controlCenterUxV1:
    "ae9aa18a07462ccab44b79250a9d28580a21ecae5ef2d3d2343d998b946c77ab",
  developerUxV2:
    "9e4c5a637c703a6f43764f9c3ec971f4b48294ac649076fddb134ccd712de0ed",
});
...
const BUSINESSES = [
  {
    ...
    source: "Identity + Business owner",
    members: [
      {
        id: "business-owner-visual",
        email: "business-owner@example.com",
...
const PRODUCTS = [
  {
    ...
    offer: {
      ...
      salesStartAt: "2026-09-01T10:00:00.000Z",
      salesEndAt: "2026-09-30T22:00:00.000Z",
      startsAt: "2026-09-21T19:30:00.000Z",
      endsAt: "2026-09-21T23:30:00.000Z",
...
const RESERVATIONS = [
  {
    businessId: "toca-do-morcego",
    inventoryLabel: "Sunset Premium",
    reservation: {
      id: "res_visual_001",
      inventoryId: "inv_visual_001",
      destinationId: "morro-de-sao-paulo",
      holderReference: "visitante.visual@example.com",
      quantity: 2,
      status: "confirmed",
      createdAt: "2026-09-21T14:15:00-03:00",
      orderId: "ord_visual_001",
      paymentId: "pay_visual_001",
      unitAmount: { minorUnits: 12500, currency: "BRL" },
      product: { kind: "business_experience", reference: "sunset-premium" },
    },
  },
...
const AUDIT = [
  {
    ...
    actorUserId: "platform-owner-visual",
    actorRole: "PLATFORM_OWNER",
    action: "business.destination.updated",
    entityType: "business",
    entityId: "toca-do-morcego",
  ...
  },
  {
    ...
    actorUserId: "platform-owner-visual",
    actorRole: "PLATFORM_OWNER",
    action: "content.preview.reviewed",
    entityType: "content",
    entityId: "place-segunda-praia",
    result: "success",
  },
  {
    ...
    actorUserId: "platform-owner-visual",
    actorRole: "PLATFORM_OWNER",
    action: "affiliate.membership.reviewed",
    entityType: "affiliate",
    entityId: "affiliate-visual-001",
    result: "success",
  },
];
...
const DASHBOARD = {
  ...
  health: {
    readiness: "degraded",
    release: {
      sha: "visual-baseline-sha",
      version: "0.1.0-visual",
      deploymentId: "visual-regression",
    },
    checks: [
      {
        name: "API administrativa",
        status: "pass",
        detail: "Contratos disponíveis",
      },
      { name: "Fila de webhooks", status: "warn", detail: "1 retry pendente" },
    ],
  },
  modules: {
    businesses: { state: "available" },
    users: { state: "available" },
    affiliates: { state: "available" },
    crm: { state: "available" },
    ticketing: { state: "available" },
    financial: { state: "available" },
    content: { state: "available" },
    destinations: { state: "available" },
    webhooks: { state: "partial" },
  },
};
...
function byDestination(rows, url, readDestination) {
  ...
  return destinationId
    ? rows.filter((row) => readDestination(row) === destinationId)
    : rows;
}
...
export function fixtureResponse(url, method) {
  ...
  if (method !== "GET")
    return { status: 405, body: { error: "VISUAL_FIXTURE_READ_ONLY" } };
  if (path === "/session")
    return {
      status: 200,
      body: { actor: USERS[0], effectiveUser: USERS[0], support: null },
    };
  if (path === "/dashboard") return { status: 200, body: DASHBOARD };
  if (path === "/destinations")
    return { status: 200, body: { destinations: DESTINATIONS } };

  const destination = path.match(/^\/destinations\/([^/]+)$/u);
  if (destination) {
    const item = DESTINATIONS.find(
      (candidate) => candidate.id === decodeURIComponent(destination[1]),
    );
    return item
      ? { status: 200, body: { data: item } }
      : { status: 404, body: { error: "DESTINATION_NOT_FOUND" } };
  }
  ...
  if (sessions)
    return {
      ...
      body: {
        sessions: [
          {
            handle: "sess_visual_000000000001",
            issuedAt: 1790000000,
            expiresAt: 1890000000,
            revokedAt: null,
          },
          {
            handle: "sess_visual_revoked_0002",
            issuedAt: 1789900000,
            expiresAt: 1890000000,
            revokedAt: 1790001000,
          },
        ],
      },
    };
  ...
  if (user) {
    const item = USERS.find(
      (candidate) => candidate.id === decodeURIComponent(user[1]),
    );
    return item
      ? { status: 200, body: { user: item } }
      ...
      ? {
          ...
          body: {
            profile: {
              ...
              specialty: "Sunset",
              description: "Experiência premium ao pôr do sol.",
              cta: "Ver empresa",
  ...
  if (path === "/affiliates") {
    ...
    return {
      ...
      body: {
        data:
          !destinationId || destinationId === "morro-de-sao-paulo"
            ? AFFILIATES
            : [],
      },
    };
    ...
    return {
      ...
      body: {
        data: {
          ...
          memberships: [
            {
              programId: "morro-creators",
              destinationId: "morro-de-sao-paulo",
              status: "approved",
              eligibleForAttribution: true,
              financialOnboardingStatus: "ready",
            },
          ],
          ...
          conversions: [
            {
              conversionId: "conv_visual_001",
              orderId: "ord_visual_001",
              eligibleRevenueMinor: 25000,
    ...
    return {
      ...
      body: {
        data: [
          {
            id: 42,
            companyName: "Pousada Horizonte",
            destinationId: "morro-de-sao-paulo",
            contactName: "Ana Costa",
            email: "ana@example.com",
            stage: "proposal",
            status: "active",
            monthlyValue: "1490.00",
          },
          {
            id: 43,
            companyName: "Restaurante Farol",
            destinationId: "morro-de-sao-paulo",
  ...
  if (path === "/products") {
    ...
    if (businessId)
      rows = rows.filter((item) => item.businessId === businessId);
    return { status: 200, body: { data: rows } };
  }

  if (path === "/reservations") {
    const businessId = url.searchParams.get("businessId");
    let rows = byDestination(
      RESERVATIONS,
      url,
      (item) => item.reservation.destinationId,
    );
    if (businessId)
      rows = rows.filter((item) => item.businessId === businessId);
    return { status: 200, body: { data: rows } };
        ...
        data: [
          {
            id: "inv_visual_001",
            label: "Sunset Premium",
            productReference: "sunset-premium",
            available: 38,
            capacity: 60,
            unitAmount: { minorUnits: 12500, currency: "BRL" },
          },
          {
            ...
            productReference: "volta-ilha",
            available: 16,
            capacity: 24,
            unitAmount: { minorUnits: 22000, currency: "BRL" },
          },
        ],
      },
    };
        ...
        data: [
          {
            ...
            fields: {
              title: "Segunda Praia",
              summary: "Praia com gastronomia, lazer e vida noturna.",
            },
          ...
          {
            id: "event-sunset-visual",
            destinationId: "morro-de-sao-paulo",
            kind: "event",
            locale: "pt-BR",
            status: "preview",
            version: 2,
            fields: { title: "Sunset em Morro" },
            sourceReference: "event:sunset",
  ...
  if (path === "/audit")
    return {
      status: 200,
      body: { durability: "mysql-append-only", entries: AUDIT },
    };
  if (path === "/system")
    return {
      status: 200,
  ...
  if (payment)
    return {
      ...
      body: {
        data: {
          id: decodeURIComponent(payment[1]),
          status: "APPROVED",
          amount: { minorUnits: 25000, currency: "BRL" },
          subject: { reference: "ord_visual_001" },
          providerReference: "mp_visual_001",
  ...
  if (order)
    return {
      ...
      body: {
        data: {
          id: decodeURIComponent(order[1]),
          status: "paid",
          source: { kind: "reservation", reference: "res_visual_001" },
          pricing: {
            amount: { minorUnits: 25000, currency: "BRL" },
            planName: "Sunset Premium",
          },
          updatedAt: "2026-09-21T16:20:00.000Z",
        },
      },
        ...
        results: [
          {
            ...
            context: "Morro de São Paulo",
            href: "#businesses:toca-do-morcego",
          },
        ],
      },
    };
  ...
  return {
    status: 404,
    body: { error: "VISUAL_FIXTURE_NOT_IMPLEMENTED", path },
  };
}
