...
async function membershipsForAffiliate(pool, affiliateId) {
  const [rows] = await pool.execute(
    ...
    await Promise.all([
      ...
      pool.execute(
        `SELECT request_id, entitlement_id, entitlement_revision, conversion_id,
                ...
                created_at, updated_at
           FROM affiliate_materialization_requests
          WHERE affiliate_id = ?
...
export function createAffiliatesApi({
  ...
  async function handleMe(request, response, correlation) {
    ...
    const projection = await commercialProjection(pool, active.account);
