...
import {
  projectPublicDestination,
  resolvePublicDestination,
} from "./destination-public-projection.mjs";
...
const owner = {
  ...
  center: { lat: -13.38, lng: -38.91, zoom: 14 },
  modules: ["map", "assistant"],
  featureFlags: { map: true, assistant: true, booking: true },
  version: 7,
  createdAt: "secret",
  updatedAt: "secret",
};
describe("public destination projection", () => {
  ...
  it("rejects suspended, malformed and foreign owner documents", () => {
    expect(
      projectPublicDestination({ ...owner, status: "suspended" }, fallback),
    ).toBeNull();
    expect(
      projectPublicDestination(
        { ...owner, center: { lat: "bad", lng: -38.91 } },
        fallback,
      ),
    ).toBeNull();
    expect(
      projectPublicDestination({ ...owner, id: "other" }, fallback),
    ).toBeNull();
  });
  it("falls back when owner runtime is unavailable or throws", async () => {
    expect(
      await resolvePublicDestination({ state: "unavailable" }, fallback),
    ).toBe(fallback);
    const runtime = {
      state: "configured",
      service: {
  ...
  it("uses active owner data through the runtime service", async () => {
    const runtime = {
      state: "configured",
      service: { read: async () => ({ destination: owner }) },
    };
    expect((await resolvePublicDestination(runtime, fallback)).name).toBe(
      "Morro Owner",
    );
  });
});
