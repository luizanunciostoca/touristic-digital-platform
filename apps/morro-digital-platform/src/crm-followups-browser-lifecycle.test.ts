...
describe("CRM M123 follow-ups browser lifecycle", () => {
  it("exposes scheduling and settings forms aligned with server limits", async () => {
    ...
    expect(page).toContain('name="maxAttempts" type="number" min="1" max="100"');
  ...
  it("uses authenticated lifecycle endpoints and server-owned transitions", async () => {
    ...
    expect(lifecycle).toContain('request("/api/crm/follow-ups/settings", "PUT"');
    expect(lifecycle).toContain('item.status === "pending" ? "sent" : "responded"');
