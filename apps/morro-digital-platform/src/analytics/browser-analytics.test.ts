...
describe("browser analytics instrumentation", () => {
  ...
  it("deduplicates category/place views and tracks tour completion duration", async () => {
    ...
    expect(inputs.filter((input) => input.name === "category_viewed")).toHaveLength(
      2,
    );
    ...
    expect(inputs.find((input) => input.name === "tour_started")?.attributes).toEqual({
      tourId: "volta-a-ilha",
      stopCount: 5,
    });
    expect(
  ...
  it("maps commerce and transaction milestones without payment amounts", async () => {
    ...
    expect(inputs.find((input) => input.name === "commerce_clicked")?.attributes).toEqual({
      surface: "assistant",
      placeId: "segunda-praia",
      offerId: "offer-001",
    });
    expect(inputs.find((input) => input.name === "checkout_started")?.attributes).toEqual({
      orderId: "order-001",
      itemCount: 2,
      currency: "BRL",
    });
    expect(inputs.find((input) => input.name === "payment_approved")?.attributes).toEqual({
      orderId: "order-001",
      currency: "BRL",
      paymentMethod: "pix",
    });
