...
const CATEGORY_ALIASES: Readonly<Record<string, readonly string[]>> =
  Object.freeze({
    beaches: ["praia", "praias", "beach", "beaches", "playa", "playas", "חוף", "חופים"],
    restaurants: [
      "restaurante",
      "restaurantes",
      "comida",
    ...
    hotels: ["pousada", "pousadas", "hotel", "hoteis", "hotels", "hoteles", "מלון", "מלונות"],
    shops: ["loja", "lojas", "compra", "compras", "shop", "shops", "tienda", "tiendas", "חנות", "חנויות"],
    attractions: [
      "atracao",
      "atracoes",
      "turismo",
    ...
    nightlife: ["noite", "balada", "bar", "bares", "nightlife", "vida nocturna", "בר", "ברים", "חיי לילה"],
    tours: ["passeio", "passeios", "tour", "tours", "paseo", "paseos", "סיור", "סיורים"],
    emergencies: [
      "emergencia",
      "emergencias",
      "hospital",
...
export function resolveAssistantV1ResidualCommand(
  ...
  if (transportQuestion && !requestedCategory) {
    return null;
  }
  ...
  if (place) return Object.freeze({ type: "map_focus_place", place });
...
export async function executeAssistantV1ResidualCommand(
  ...
  const blocksCamera =
    ...
    command.type === "map_filter_category" ||
  ...
  if (command.type === "map_filter_category") {
    if (map) bumpCameraGeneration(map);
    const success =
  ...
  if (command.type === "map_style") {
    ...
    const cameraGeneration = bumpCameraGeneration(map);
    map.setStyle(styleUrl);
    if (map.once && map.flyTo && center) {
      map.once("style.load", () => {
        if (!isCurrentCameraGeneration(map, cameraGeneration)) return;
        map.flyTo?.({
  ...
  if (command.type === "map_zoom") {
    ...
    const target =
      command.direction === "in"
        ? Math.min(current + 2, maxZoom)
        : Math.max(current - 2, minZoom);
