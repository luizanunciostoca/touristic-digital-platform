...
describe("assistant domain i18n copy", () => {
  it.each(languages)("provides localized help and place options in %s", (language) => {
  ...
  it.each(languages)("localizes favorites/photos/hours/more-info/price in %s", (language) => {
    expect(favoritesCopy(language, [])).toBeTruthy();
    expect(favoritesCopy(language, ["Segunda Praia"])).toContain("Segunda Praia");
    expect(photosCopy(language, "resolved", "Segunda Praia", 3)).toContain("3");
    expect(hoursCopy(language, "Segunda Praia", true)).toContain("Segunda Praia");
    expect(moreInfoUnavailable(language, "Segunda Praia")).toContain("Segunda Praia");
  ...
  it("keeps representative copy language-specific", () => {
    ...
    expect(locationCopy("pt").resolved).toBe("Localização atualizada com sucesso.");
  });
});
