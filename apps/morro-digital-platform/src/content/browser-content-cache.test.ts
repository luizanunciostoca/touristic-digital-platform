...
describe("browser content snapshot storage", () => {
  ...
  it("fails soft when Cache Storage cannot be opened", async () => {
    const caches = {
      open: vi.fn(async () => {
        throw new Error("cache unavailable");
      }),
    } as unknown as CacheStorage;
    const storage = createBrowserContentSnapshotStorage(
      caches,
      "https://morro.example",
    );

    await expect(storage.read("morro-de-sao-paulo")).resolves.toBeNull();
    await expect(storage.write(snapshot())).resolves.toBeUndefined();
    await expect(storage.clear("morro-de-sao-paulo")).resolves.toBeUndefined();
  });

  it("fails soft when cached reads or writes reject", async () => {
    ...
    const readStorage = createBrowserContentSnapshotStorage(
      {
        open: vi.fn(async () => readFailureCache),
      } as unknown as CacheStorage,
      "https://morro.example",
    );
    const writeStorage = createBrowserContentSnapshotStorage(
      {
        open: vi.fn(async () => writeFailureCache),
      } as unknown as CacheStorage,
      "https://morro.example",
    );

    await expect(readStorage.read("morro-de-sao-paulo")).resolves.toBeNull();
    await expect(writeStorage.write(snapshot())).resolves.toBeUndefined();
  });
...
describe("content snapshot loading", () => {
  ...
  it("keeps a valid network snapshot usable when storage rejects", async () => {
    const value = snapshot();
    const storage = {
      read: vi.fn(async () => {
  ...
  it("returns unavailable when both storage and network fail", async () => {
    const storage = {
      read: vi.fn(async () => {
        throw new Error("read failed");
      }),
