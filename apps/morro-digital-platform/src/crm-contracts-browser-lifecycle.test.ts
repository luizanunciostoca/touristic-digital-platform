...
describe("CRM M122 contracts browser lifecycle", () => {
  it("exposes a contract creation form aligned with server limits", async () => {
    ...
    expect(page).toContain('name="leadId" type="number" min="1" step="1" required');
    expect(page).toContain('name="proposalId" type="number" min="1" step="1"');
    expect(page).toContain('name="title" required maxlength="180"');
    expect(page).toContain('name="content" required maxlength="100000"');
  });

  it("uses authenticated create, send and cancel endpoints while keeping server transitions authoritative", async () => {
    const clientUrl = new URL("../../admin-crm/public/contracts.js", import.meta.url);
    ...
    expect(client).toContain("/api/crm/contracts/${contract.id}/${action.suffix}");
