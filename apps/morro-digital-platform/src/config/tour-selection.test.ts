...
describe("createMorroTourSelectionController", () => {
  it("replaces markers and publishes the selected tour", async () => {
  ...
  it("rejects unknown tours before changing the map", async () => {
    const engine = createEngine();
    const controller = createMorroTourSelectionController({
      engine,
      events: new EventBus(),
  ...
  it("rolls back markers when a TourSelected observer fails", async () => {
    const engine = createEngine();
    const events = new EventBus();
