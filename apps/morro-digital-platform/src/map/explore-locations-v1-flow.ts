...
const SUBCATEGORY_OPTIONS: Readonly<Record<string, readonly V1ExploreOption[]>> =
  Object.freeze({
...
function hasTag(location: MorroV1SearchCatalogItem, ...tags: string[]): boolean {
  const haystack = normalizedTags(location);
  return tags.some((tag) => haystack.includes(normalizeSearchText(tag)));
}

function hasArea(location: MorroV1SearchCatalogItem, ...areas: string[]): boolean {
  const area = normalizeSearchText(location.area ?? "");
  return areas.some((candidate) => area === normalizeSearchText(candidate));
}

function nameIncludes(location: MorroV1SearchCatalogItem, value: string): boolean {
  return normalizeSearchText(location.name).includes(normalizeSearchText(value));
}
  ...
  const matches = (location: MorroV1SearchCatalogItem): boolean => {
    switch (`${category}:${filter}`) {
      ...
      case "restaurants:pizza":
        return hasTag(location, "pizza", "italiana") || nameIncludes(location, "pizza");
      ...
      case "hotels:frente a praia":
        return hasArea(location, "praia") || hasTag(location, "frente_praia", "praia");
      case "hotels:na vila":
        return hasArea(location, "vila") || hasTag(location, "vila");
      ...
      case "hotels:pousada":
        return hasTag(location, "pousada", "charme") || nameIncludes(location, "pousada");
      case "shops:roupa":
        return hasTag(location, "roupas", "moda");
      case "shops:supermercado":
        return hasTag(location, "mercado", "alimentos") || nameIncludes(location, "super") || nameIncludes(location, "mercad");
      ...
      case "shops:farmacia":
        return hasTag(location, "farmacia") || nameIncludes(location, "farmacia");
