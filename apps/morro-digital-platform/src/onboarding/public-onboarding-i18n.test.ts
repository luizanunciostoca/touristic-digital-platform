...
describe("public onboarding i18n", () => {
  ...
  it("provides complete localized first-run and four-step V2 tour copy without retired Assistant shortcuts", () => {
    const expectations = [
      ["pt", "Bem-vindo ao Morro Digital", "Passo 1 de 4"],
      ["en", "Welcome to Morro Digital", "Step 1 of 4"],
      ["es", "Bienvenido a Morro Digital", "Paso 1 de 4"],
      ["he", "ברוכים הבאים ל-Morro Digital", "שלב 1 מתוך 4"],
    ] as const;

    for (const [locale, title, step] of expectations) {
      const copy = getPublicOnboardingCopy(locale);
      expect(copy.title).toBe(title);
      expect(copy.tour.steps).toHaveLength(4);
      expect(copy.tour.step(1, 4)).toBe(step);
      expect(
