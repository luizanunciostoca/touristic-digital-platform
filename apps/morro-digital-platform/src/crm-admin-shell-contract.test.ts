...
describe("CRM M104 browser shell", () => {
  it(
    "exposes the frozen V1 primary modules with truthful current capabilities",
    async () => {
      ...
      expect(html).toContain(
        "Consulta + agendamento + histórico + configurações",
      );
      expect(html).toContain(
        "Consulta + criação + conversão + cancelamento + expiração",
      );
      expect(html).toContain(
        "Consulta + cadastro + vínculo + contato + conversão/perda",
      );
