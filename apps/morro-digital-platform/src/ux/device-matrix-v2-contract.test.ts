...
describe("UX Design V2 device matrix contract", () => {
  it(
    "covers every required canonical viewport plus relevant landscape orientations",
    async () => {
  ...
  it(
    "checks overflow, touch targets, dynamic viewport units and keyboard-like compaction",
    async () => {
      const workflow = await readRepository(
        ".github/workflows/v1-home-responsive-browser-regression.yml",
      );
