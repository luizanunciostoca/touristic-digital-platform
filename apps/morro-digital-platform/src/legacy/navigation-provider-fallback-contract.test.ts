...
const browserEntryPath = fileURLToPath(new URL("../browser-entry.ts", import.meta.url));
...
describe("V1 navigation provider fallback contract", () => {
  ...
  it("destroys the Mapbox navigation runtime before entering a map fallback", async () => {
    ...
    const fallbackStart = source.indexOf("function prepareMapContainerForFallback(): void {");
    const fallbackEnd = source.indexOf("async function startBrowserWithProvider", fallbackStart);
    ...
    expect(fallbackSource.indexOf("clearBrowserNavigationRuntime();")).toBeLessThan(
      fallbackSource.indexOf("activeRealMap = undefined;"),
    );
  });
