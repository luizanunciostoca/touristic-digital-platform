...
async function load() {
  ...
  elements.reserve.href = `/apps/morro-digital-platform/public/tickets.html?offer=${encodeURIComponent(offer.id)}`;
  elements.reserve.textContent = Number(offer.availableQuantity || 0) > 0 ? "Reservar agora" : "Ver disponibilidade";
...
void load().catch(() => {
  showError("Não foi possível carregar esta experiência agora. Tente novamente em instantes.");
});
