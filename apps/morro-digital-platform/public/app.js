...
const points = [
  ...
  { name: "Igreja Nossa Senhora da Luz", coords: [-13.3775, -38.9153], icon: "✦" },
  { name: "Praça Aureliano Lima", coords: [-13.3782, -38.9148], icon: "●" },
  ...
  { name: "Fortaleza de Tapirandu", coords: [-13.3754, -38.9187], icon: "◆" },
];
...
for (const point of points) {
  ...
  L.marker(point.coords, { icon })
    ...
    .bindPopup(`<strong>${point.name}</strong><br />Explore este local no Morro Digital.`);
}

let usingTopo = false;
document.querySelector("#zoomInButton").addEventListener("click", () => map.zoomIn());
document.querySelector("#zoomOutButton").addEventListener("click", () => map.zoomOut());
...
document.querySelector("#assistantForm").addEventListener("submit", (event) => {
  ...
  assistantCard.hidden = false;
  assistantOpen.classList.remove("is-visible");
  assistantCard.querySelector("strong").textContent = "Estou preparando esta experiência para você.";
  assistantCard.querySelector("span").textContent = `Sua pergunta foi: “${question}”. A integração completa do assistente será conectada durante a migração funcional.`;
  input.value = "";
});
