...
const candidate = '77b2b097174b1c871c757705c21c1f92d323be43';
...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      globalThis.__PROVIDER77B = { verified: null, failed: null };
      addEventListener('businessPaymentVerified', event => { globalThis.__PROVIDER77B.verified = event.detail; });
      addEventListener('businessPaymentVerificationFailed', event => { globalThis.__PROVIDER77B.failed = event.detail; });
    });
    await page.waitForTimeout(2200);

    const acceptedAt = new Date().toISOString();
    const sessionId = `provider_77b_${Date.now()}`;
    const handoff = {
      sessionId,
    ...
    const outcome = await poll({
      ...
      read: () => page.evaluate(() => globalThis.__PROVIDER77B),
    ...
    console.log('PROVIDER_77B_CARD_E2E=' + JSON.stringify({
      candidate,
      staging: true,
      realMoney: false,
      realCard: false,
  ...
  console.error('PROVIDER_77B_CARD_E2E_FAILED=' + (error?.stack || error));
  process.exit(1);
});
