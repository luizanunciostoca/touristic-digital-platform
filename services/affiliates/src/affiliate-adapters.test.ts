...
describe("Durable Affiliate -> Financial materialization acceptance", () => {
  it("converges exact replay through durable readback without a second Financial mutation", async () => {
    ...
    const replay = await new DurableFinancialMaterializationAdapter(
      memory.repository,
      financial,
      clock(),
  ...
  it("fails closed when another requestId collides with the same entitlement revision", async () => {
    const memory = memoryRepository();
    const financial = financialDouble();
    const adapter = new DurableFinancialMaterializationAdapter(
      memory.repository,
  ...
  it("uses authoritative Financial readback before retrying uncertain delivery", async () => {
    const memory = memoryRepository();
    ...
    const result = await new DurableFinancialMaterializationAdapter(
      ...
      financial,
      clock(),
    ).requestMaterialization(pending);
  ...
  it("detects divergent concurrent Financial outcomes instead of masking them", async () => {
    const memory = memoryRepository();
    const pending = request();
    memory.records.set(pending.requestId, {
      requestId: pending.requestId,
    ...
    financial.readMaterialization.mockImplementationOnce(async () => {
      const current = memory.records.get(pending.requestId);
      if (!current) throw new Error("TEST_RECORD_REQUIRED");
      memory.records.set(pending.requestId, {
        ...current,
        state: "accepted",
    ...
    await expect(
      new DurableFinancialMaterializationAdapter(
        memory.repository,
        financial,
        clock(),
      ).requestMaterialization(pending),
    ).rejects.toThrow("AFFILIATE_MATERIALIZATION_RESULT_CONFLICT");
  });
});
