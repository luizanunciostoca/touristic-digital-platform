...
async function seedCommercialEvidence(
  ...
  await pool.execute(
    `INSERT INTO affiliate_conversions
     ...
     VALUES (?, ?, ?, ?, ?, ?, UNHEX(?), 10000, 'BRL', ?, ?, 'initial_purchase', 'AFFILIATE-POLICY-V1', ?)`,
    [
      conversionId,
      input.attributionId,
      input.affiliateId,
  ...
  await pool.execute(
    `INSERT INTO affiliate_materialization_requests
     ...
     VALUES (?, ?, 1, ?, ?, 'AFFILIATE-POLICY-V1', UNHEX(?), ?, 'accepted', ?, NULL, 0, 1, ?, ?)`,
    [
...
describe.skipIf(!databaseUrl)("Affiliates Privacy/LGPD MySQL acceptance", () => {
