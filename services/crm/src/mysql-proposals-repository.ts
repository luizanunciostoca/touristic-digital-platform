  ...
  implements CrmProposalBoundaryRepository
{
  ...
  async create(record: CrmProposalCreateRecord): Promise<CrmProposal> {
    const [result] = await this.pool.execute<ResultSetHeader>(
      `INSERT INTO crm_proposals (lead_id, title, plan_name, monthly_value, setup_fee, trial_days, features, custom_message, share_token, status, valid_until, created_by_subject)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        record.leadId,
        record.title,
