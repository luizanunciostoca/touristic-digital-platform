...
describe("CRM M71 MySQL persistence", () => {
  it("freezes server-only identity and relational constraints in the schema", () => {
    expect(crmM71SchemaSql).toContain("assigned_to_subject VARCHAR(191) NOT NULL");
  ...
  it("uses prepared placeholders and bounded pagination for list queries", async () => {
    ...
    await repository.list({ stage: "new_lead", status: "active", search: "Toca", limit: 25, offset: 5 });
    expect(calls).toHaveLength(1);
    expect(calls[0]?.sql).toContain("stage = ?");
    expect(calls[0]?.sql).toContain("LIMIT ? OFFSET ?");
    expect(calls[0]?.sql).not.toContain("Toca");
    expect(calls[0]?.values).toEqual(["new_lead", "active", "%Toca%", "%Toca%", "%Toca%", 25, 5]);
  });

  it("reads back the generated id before returning a created lead", async () => {
    const now = new Date("2026-08-12T00:00:00Z");
    const row = {
      id: 42, company_name: "Toca", segment: null, contact_name: null, phone: null, whatsapp: null, email: null, address: null, website: null, notes: null, stage: "new_lead", status: "active", source: null, referred_by_id: null, monthly_value: null, created_at: now, updated_at: now, last_contact_at: null, converted_at: null,
    };
    const { pool, calls } = poolFixture([{ insertId: 42 }, [row]]);
    const repository = new MySqlCrmLeadRepository(pool as never);
    const created = await repository.create({ companyName: "Toca", assignedToSubject: "auth0|owner", stage: "new_lead", status: "active" });
  ...
  it("initializes the frozen checklist idempotently with the real lead id", async () => {
    ...
    expect(calls.every((call) => call.sql.includes("INSERT IGNORE"))).toBe(true);
  ...
  it("persists interaction metadata as data, never interpolated SQL", async () => {
    const { pool, calls } = poolFixture();
    const repository = new MySqlCrmLeadRepository(pool as never);
    await repository.appendInteraction({ leadId: 9, type: "stage_change", content: "Moved", actorSubject: "subject-1", metadata: { from: "new_lead", to: "first_contact" } });
