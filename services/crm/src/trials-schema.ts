...
CREATE TABLE IF NOT EXISTS crm_trials (
  ...
  INDEX crm_trials_notification_lease_idx (status, notified_at, notification_claimed_at, end_date),
...
export const crmM95TrialsNotificationLeaseSchemaSql = `
ALTER TABLE crm_trials
  ADD COLUMN notification_claimed_at TIMESTAMP(3) NULL AFTER notification_task_uid,
  ADD INDEX crm_trials_notification_lease_idx (status, notified_at, notification_claimed_at, end_date);
`;
