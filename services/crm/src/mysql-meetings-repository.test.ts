...
describe("CRM M74 MySQL meetings persistence", () => {
  it(
    "freezes the V1 meeting vocabulary with relational and subject identity constraints",
    () => {
  ...
  it(
    "reads back generated meeting ids and persists stable actor subject",
    async () => {
  ...
  it(
    "persists meeting interactions without interpolating actor or content",
    async () => {
      const { pool, calls } = poolFixture();
      const repository = new MySqlCrmMeetingRepository(pool as never);
