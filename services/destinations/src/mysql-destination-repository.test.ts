...
function row(overrides = {}) {
  return {
    ...
    currency: "BRL",
    branding_json: JSON.stringify({ name: "Morro Digital", shortName: "Morro", tagline: "Descubra Morro" }),
...
describe("MySqlDestinationRepository", () => {
  ...
  it("fails closed for corrupted persisted configuration", async () => {
    const pool = {
      execute: async () => [[row({ center_json: JSON.stringify({ lat: 999, lng: 0, zoom: 1 }) })], []],
    };
    const repository = new MySqlDestinationRepository(pool as never);
  ...
  it("uses compare-and-swap version on replacement", async () => {
    ...
    const current = {
      ...
      branding: { name: "Morro Digital", shortName: "Morro", tagline: "Descubra Morro" },
      center: { lat: -13.3781, lng: -38.9132, zoom: 13.5 },
      modules: ["explore"], featureFlags: { assistant: true },
      version: 1, createdAt: "2026-09-20T23:00:00.000Z", updatedAt: "2026-09-20T23:00:00.000Z",
    } as const;
    await expect(repository.replace(current, { ...current, version: 2 })).resolves.toBe(true);
