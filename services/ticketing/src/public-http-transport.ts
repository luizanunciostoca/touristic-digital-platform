...
export class TicketingPublicHttpTransport {
  constructor(
    private readonly dependencies: TicketingPublicHttpTransportDependencies,
  ) {}
  ...
  async handle(request: TicketingHttpRequest): Promise<TicketingHttpResponse> {
    ...
    if (!this.dependencies.enabled) {
      return response(
        503,
        { error: "TICKETING_FEATURE_DISABLED" },
        correlation,
      );
    }
    ...
    if (relative === "/offline-sync" && method === "POST") {
      try {
        ...
        if (!body)
          return response(400, { error: "INVALID_REQUEST" }, correlation);
        const result = await this.dependencies.offlineDevices.sync({
          credentialToken: header(request, "authorization").replace(
            /^Bearer\s+/iu,
            "",
          ),
    ...
    try {
      ...
      if (relative === "/reservations" && method === "GET") {
        const reservations =
          await this.dependencies.reads.listReservationsByHolderReference(
            actor.subject,
          );
        return response(
          200,
      ...
      if (relative === "/reservations" && method === "POST") {
        ...
        const now = canonicalNow(this.dependencies.clock);
        if (
          !IDEMPOTENCY_REFERENCE.test(reference) ||
          !holderInput ||
          !returnUrl
        ) {
          return response(
        ...
        try {
          profile = await this.dependencies.holders.save(
            candidate as TicketHolderProfile,
          );
        } catch {
          profile = null;
        }
        if (!profile)
          return response(
            400,
            { error: "INVALID_HOLDER_PROFILE" },
            correlation,
          );
        const held: TicketReservationHoldResult =
          await this.dependencies.reservations.hold({
        ...
        const amount = createMoney(
          totalMinorUnits,
          held.reservation.unitAmount.currency,
        );
        if (!amount || !Number.isSafeInteger(totalMinorUnits)) {
          throw new Error("TICKETING_RESERVATION_AMOUNT_INVALID");
        }
        const orderResult =
          await this.dependencies.reservationOrders.placeReservationOrder({
        ...
        await this.dependencies.audit.record({
          ...
          result: "success",
          reason:
            held.replayed || orderResult.replayed ? "replayed" : "created",
          actorSubject: actor.subject,
          correlationId: correlation,
          reservationId: held.reservation.id,
        });
        return response(
          held.replayed ? 200 : 201,
          {
            data: Object.freeze({
      ...
      const reservationMatch =
        /^\/reservations\/(trv_[A-Za-z0-9_-]+)(?:\/(ticket|cancel))?$/u.exec(
          relative,
        );
      if (reservationMatch?.[1]) {
        ...
        if (!reservation)
          return response(404, { error: "RESERVATION_NOT_FOUND" }, correlation);
        const action = reservationMatch[2] ?? "read";
        if (action === "read" && method === "GET") {
          return response(
            200,
            { data: publicReservation(reservation) },
            correlation,
          );
        }
        if (action === "cancel" && method === "POST") {
          ...
          return response(
            200,
            { data: publicReservation(cancelled.reservation) },
            correlation,
          );
        }
        if (action === "ticket" && method === "GET") {
          ...
          const tickets = await this.dependencies.tickets.findByOrderId(
            reservation.orderId,
          );
          const ticket =
            tickets.find(
              (entry) => entry.paymentId === reservation.paymentId,
            ) ?? null;
          if (!ticket)
            return response(404, { error: "TICKET_NOT_FOUND" }, correlation);
          return response(
            200,
            {
      ...
      if (relative === "/operator/check-in" && method === "POST") {
        const body = record(request.body);
        if (!body)
          return response(400, { error: "INVALID_CHECKIN" }, correlation);
      ...
      if (relative === "/operator/offline-devices" && method === "POST") {
        ...
        if (
          ...
          return response(
            400,
            { error: "INVALID_DEVICE_REQUEST" },
            correlation,
          );
        }
        const issuedAt = canonicalNow(this.dependencies.clock);
        const expiresAt = new Date(
          Date.parse(issuedAt) + ttlSeconds * 1_000,
        ).toISOString();
      ...
      const revokeMatch =
        /^\/operator\/offline-devices\/(tdv_[A-Za-z0-9_-]+)\/revoke$/u.exec(
          relative,
        );
