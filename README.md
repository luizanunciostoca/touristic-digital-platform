...
`@touristic/financial` e `@touristic/ordering` começaram no M136 como packages de domínio/ports. O M137 adicionou `@touristic/financial-server` e `@touristic/ordering-server` com persistência MySQL durável. O M138 acrescentou o application service provider-neutral, revalidação do handoff, pricing authority server-only e composição recuperável entre os dois bancos. O M139 adiciona a fronteira HTTP versionada, Auth/CSRF/tenant, capability convidada assinada, status público protegido por token cujo hash é o único valor persistido, allowlist de retorno, auditoria estruturada e rate limiting conservador. Ainda não existem provider financeiro, webhook público, checkout browser ou movimentação monetária; esse progresso não significa disponibilidade de pagamentos em produção.
